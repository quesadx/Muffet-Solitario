...
public class Formato {
    ...
    public DateTimeFormatter formatDateShort =
        DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT);
    public DateTimeFormatter formatDateMedium =
    ...
    public TextFormatter twoDecimalFormat() {
        TextFormatter numericFormat = new TextFormatter<>(c -> {
            ...
            if (c.getControlNewText().contains(",")) {
                ...
                Object object = decimalFormat.parse(
                    c.getControlNewText(),
                    parsePosition
                );
                ...
                if (
                    ...
                    Pattern validDoubleText = Pattern.compile(
                        "^[0-9]*+(\\.[0-9]{0,2})?$"
                    );
                    if (
                        validDoubleText
                            .matcher(c.getControlNewText().replace(",", ""))
                            .matches()
                    ) {
                        return c;
                    } else {
    ...
    public TextFormatter cedulaFormat(Integer maxLength) {
        TextFormatter<String> cedulaFormat = new TextFormatter<>(c -> {
            ...
            if (maxLength > 0) {
                if (
                    ((TextInputControl) c.getControl()).getLength() >=
                        maxLength &&
                    !c.isDeleted()
                ) {
                    return null;
    ...
    public TextFormatter maxLengthFormat(Integer length) {
        TextFormatter maxLengthFormat = new TextFormatter<>(c -> {
            ...
            if (
                ((TextInputControl) c.getControl()).getLength() >= length &&
                !c.isDeleted()
            ) {
                return null;
            }
